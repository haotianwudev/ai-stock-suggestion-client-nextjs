...
const Card = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <div className={`bg-white dark:bg-[#0A0D14] rounded-xl shadow-lg border border-slate-100 dark:border-slate-800 overflow-hidden ${className}`}>
    {children}
  </div>
);
...
const Badge = ({ children, color = "blue" }: { children: React.ReactNode; color?: string }) => {
  const colors: Record<string, string> = {
    blue: "bg-blue-100 text-[#A8672E] dark:text-[#D08F52] border-blue-200",
    purple: "bg-purple-100 text-purple-700 border-purple-200",
    rose: "bg-rose-100 text-[#BC4128] dark:text-[#E2694A] border-rose-200",
    emerald: "bg-emerald-100 text-[#1D8A70] dark:text-[#3CBF9C] border-emerald-200",
...
const SectionTitle = ({ title, subtitle, icon: Icon, color = "text-slate-800 dark:text-slate-200" }: {
  title: string;
  subtitle: string;
  ...
  <div className="mb-12 text-center max-w-4xl mx-auto">
    <div className="flex justify-center mb-4">
      <div className="p-3 bg-white dark:bg-[#0A0D14] rounded-full shadow-md">
        {Icon && <Icon className={`w-8 h-8 ${color}`} />}
      </div>
    </div>
    <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-slate-100 mb-4 font-serif">{title}</h2>
    ...
    <div className="p-6 flex-1 flex flex-col">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 leading-tight font-serif">{title}</h3>
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">{subtitle}</p>
          </div>
        </div>
      </div>
      <p className="text-slate-600 dark:text-slate-400 mb-6 text-sm leading-relaxed min-h-[60px]">{description}</p>
      <div className="mb-6">
        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Best Application</span>
        <div className="bg-slate-50 dark:bg-[#14171B] p-2 rounded border border-slate-100 dark:border-slate-800 text-xs font-medium text-slate-700 dark:text-slate-300">{bestFor}</div>
      </div>
      <div className="space-y-4 mt-auto">
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-[#1D8A70] dark:text-[#3CBF9C] mb-2 flex items-center gap-1">
            <CheckCircle className="w-3 h-3" /> Advantages
          </h4>
          <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1.5 pl-2 border-l-2 border-emerald-100">
            {pros.map((pro, i) => <li key={i}>{pro}</li>)}
          </ul>
        </div>
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-[#BC4128] dark:text-[#E2694A] mb-2 flex items-center gap-1">
            <AlertTriangle className="w-3 h-3" /> Limitations
          </h4>
          <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1.5 pl-2 border-l-2 border-rose-100">
...
const WorkflowStep = ({ number, title, description, methods, checks, isLast }: WorkflowStepProps) => (
  <div className="relative flex group">
    ...
    <div className="pb-12 pt-1 flex-1">
      <div className="bg-white dark:bg-[#0A0D14] p-6 rounded-xl shadow-md border border-slate-100 dark:border-slate-800 group-hover:border-indigo-100 transition-colors">
        ...
        {checks && (
          <div className="bg-slate-50 dark:bg-[#14171B] rounded-lg p-4 border border-slate-100 dark:border-slate-800">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {checks.map((check, i) => (
                <div key={i} className="flex items-center gap-2 text-xs font-medium text-slate-700 dark:text-slate-300">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#A8672E] dark:bg-[#D08F52]"></div>
                  {check}
                </div>
              ))}
            </div>
...
const DeepDiveSection = ({ title, icon: Icon, children, color = "blue" }: DeepDiveSectionProps) => {
  const bgColors: Record<string, string> = {
    blue: "bg-[#A8672E]/10 dark:bg-[#D08F52]/10",
    amber: "bg-amber-50",
    rose: "bg-[#BC4128]/10 dark:bg-[#E2694A]/10",
    indigo: "bg-[#A8672E]/10 dark:bg-[#D08F52]/10"
  };
    ...
    >
      <div className="max-w-7xl mx-auto px-6 text-slate-800 dark:text-slate-200">
        ...
        <div className="space-y-24">
          ...
          <section>
            <div className="grid md:grid-cols-3 gap-6">
              <Card className="p-8 border-t-4 border-t-indigo-500">
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-3 flex items-center gap-2 font-serif">
                  <AlertOctagon className="w-5 h-5 text-[#A8672E] dark:text-[#D08F52]" />
              ...
              <Card className="p-8 border-t-4 border-t-indigo-500">
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-3 flex items-center gap-2 font-serif">
                  <ShieldCheck className="w-5 h-5 text-[#A8672E] dark:text-[#D08F52]" />
                  The Robustness Goal
                </h3>
                <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
              ...
              <Card className="p-8 border-t-4 border-t-indigo-500">
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-3 flex items-center gap-2 font-serif">
                  <Sigma className="w-5 h-5 text-[#A8672E] dark:text-[#D08F52]" />
          ...
          <section id="taxonomy">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <SimulationMethod
                ...
                icon={Cpu}
                colorClass="bg-[#BC4128] dark:bg-[#E2694A]"
          ...
          <section className="bg-white dark:bg-[#0A0D14] rounded-3xl p-8 md:p-12 shadow-sm border border-slate-200 dark:border-slate-800">
            <SectionTitle
            ...
            <div className="grid lg:grid-cols-2 gap-12">
              <div>
                <DeepDiveSection title="The Cone of Uncertainty" icon={Maximize2} color="indigo">
                  ...
                  <ul className="space-y-3">
                    <li className="flex gap-3 text-sm text-slate-700 dark:text-slate-300">
                      <span className="font-bold text-[#A8672E] dark:text-[#D08F52]">Median Path (50%):</span>
                      Expected performance if the future resembles the past average.
                    </li>
                    <li className="flex gap-3 text-sm text-slate-700 dark:text-slate-300">
                      <span className="font-bold text-[#A8672E] dark:text-[#D08F52]">The 5th Percentile:</span>
                      The &ldquo;Bad Luck&rdquo; boundary. If your live strategy falls below this line, it is likely broken, not just unlucky.
                    </li>
                    <li className="flex gap-3 text-sm text-slate-700 dark:text-slate-300">
                      <span className="font-bold text-[#A8672E] dark:text-[#D08F52]">The 99th Percentile Drawdown:</span>
                ...
                <DeepDiveSection title="Sequence Risk Analysis" icon={Minimize2} color="rose">
                  ...
                  <div className="bg-white dark:bg-[#0A0D14] p-4 rounded-lg border border-rose-100 shadow-sm">
              ...
              <div className="space-y-8">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-4 font-serif">Quantitative Pitfalls</h3>
                  <p className="text-slate-600 dark:text-slate-400 mb-6">Common errors that invalidate Monte Carlo results.</p>
                  <div className="space-y-4">
                    <div className="flex gap-4 p-4 bg-amber-50 rounded-xl border border-amber-100">
                      <AlertTriangle className="w-6 h-6 text-amber-600 flex-shrink-0" />
                      <div>
                        <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm">Look-Ahead Bias in Blocks</h4>
                    ...
                    <div className="flex gap-4 p-4 bg-amber-50 rounded-xl border border-amber-100">
                      <AlertTriangle className="w-6 h-6 text-amber-600 flex-shrink-0" />
                      <div>
                        <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm">Breaking Serial Correlation</h4>
                        <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                    ...
                    <div className="flex gap-4 p-4 bg-amber-50 rounded-xl border border-amber-100">
                      <AlertTriangle className="w-6 h-6 text-amber-600 flex-shrink-0" />
                      <div>
                        <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm">Distribution Mismatch</h4>
                        <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                ...
                <div className="p-6 bg-slate-900 text-white rounded-xl shadow-xl">
                  <div className="flex items-center gap-2 mb-4">
                    <FileText className="w-5 h-5 text-[#A8672E] dark:text-[#D08F52]" />
                  ...
                  <div className="flex items-center justify-between text-xs border-t border-slate-700 pt-4">
                    ...
                    <span className="font-mono text-[#1D8A70] dark:text-[#3CBF9C]">Sharpe: 1.8</span>
                  </div>
                  <div className="flex items-center justify-between text-xs mt-2">
                    <span>Strategy Performance on Noise Data</span>
                    <span className="font-mono text-[#BC4128] dark:text-[#E2694A]">Sharpe: 0.1</span>
                  </div>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <Card className="p-8 border-l-4 border-l-indigo-600">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 font-serif">Trend Following</h3>
                ...
                <div className="bg-slate-50 dark:bg-[#14171B] p-4 rounded-lg space-y-3">
                  <div className="flex justify-between text-xs">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Recommended Method:</span>
                    <span className="text-[#A8672E] dark:text-[#D08F52]">Stationary Block Bootstrap (SBBS)</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Block Size:</span>
                    <span className="text-slate-600 dark:text-slate-400">Avg Trend Duration (e.g., 20-60 days)</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Key Metric:</span>
                    <span className="text-[#BC4128] dark:text-[#E2694A]">Drawdown Duration</span>
                  </div>
                </div>
              </Card>
              <Card className="p-8 border-l-4 border-l-emerald-600">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 font-serif">HFT / Mean Reversion</h3>
                ...
                <div className="bg-slate-50 dark:bg-[#14171B] p-4 rounded-lg space-y-3">
                  <div className="flex justify-between text-xs">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Recommended Method:</span>
                    <span className="text-[#A8672E] dark:text-[#D08F52]">Execution Randomization + Spread Jitter</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Noise Injection:</span>
                    <span className="text-slate-600 dark:text-slate-400">Add 10-20% of spread to every fill</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Key Metric:</span>
                    <span className="text-[#BC4128] dark:text-[#E2694A]">Break-even Win Rate</span>
                  </div>
                </div>
              </Card>
              <Card className="p-8 border-l-4 border-l-purple-600 lg:col-span-2">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 font-serif">Multi-Asset Portfolio</h3>
                ...
                <div className="bg-slate-50 dark:bg-[#14171B] p-4 rounded-lg grid md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div className="flex justify-between text-xs">
                      <span className="font-bold text-slate-700 dark:text-slate-300">Method:</span>
                      <span className="text-[#A8672E] dark:text-[#D08F52]">Joint Block Bootstrap</span>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between text-xs">
                      <span className="font-bold text-slate-700 dark:text-slate-300">Stress Test:</span>
                      <span className="text-[#BC4128] dark:text-[#E2694A]">Correlation Breakdown</span>
                    </div>
                    <p className="text-xs text-slate-500 italic">

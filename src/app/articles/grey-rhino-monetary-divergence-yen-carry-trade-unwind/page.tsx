...
import { ArrowLeft, TrendingUp, TrendingDown, AlertTriangle, Globe, ArrowRight, DollarSign, Activity, BarChart2, Zap, Clock, Shield, Briefcase, Layers, RefreshCw, ChevronRight, Info, Music } from 'lucide-react';
import { articles } from '@/data/articles';
import { StructuredData, BreadcrumbStructuredData } from '@/components/seo/structured-data';

export default function GreyRhinoArticle() {
  const [activeTab, setActiveTab] = useState('overview');
  const currentArticle = articles.find(article => article.slug === 'grey-rhino-monetary-divergence-yen-carry-trade-unwind');
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans selection:bg-indigo-100">
        ...
        {/* Hero Section */}
        <div className="bg-gradient-to-br from-indigo-900 via-indigo-800 to-violet-900 text-white pt-16 pb-32 px-6 md:px-12 lg:px-24 relative overflow-hidden">
          {/* Abstract Shapes */}
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-white opacity-5 rounded-full blur-[100px] translate-x-1/2 -translate-y-1/2"></div>
          <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-indigo-400 opacity-10 rounded-full blur-[80px] -translate-x-1/3 translate-y-1/3"></div>

          <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex items-center gap-2 mb-6">
                <span className="px-3 py-1 rounded-full bg-indigo-500/30 border border-indigo-400/30 text-indigo-100 text-xs font-semibold backdrop-blur-sm uppercase tracking-wide">Special Report</span>
                <span className="px-3 py-1 rounded-full bg-rose-500/30 border border-rose-400/30 text-rose-100 text-xs font-semibold backdrop-blur-sm flex items-center gap-1">
                  <AlertTriangle size={12} /> High Alert
                </span>
              </div>
              <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight mb-6 leading-tight">
                The Grey Rhino <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-200 to-violet-100">Monetary Divergence</span>
              </h1>
              <p className="text-indigo-100 text-lg max-w-xl leading-relaxed opacity-90 mb-8">
            ...
            <div className="hidden lg:grid grid-cols-2 gap-4 opacity-90">
              <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10">
                <p className="text-indigo-200 text-sm mb-1">Carry Trade Exposure</p>
                <p className="text-3xl font-bold text-white">$20 Trillion</p>
                <p className="text-xs text-indigo-300 mt-2">Est. Global Gross Notional</p>
              </div>
              <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10">
                <p className="text-indigo-200 text-sm mb-1">Spread Compression</p>
                <p className="text-3xl font-bold text-white">-125 bps</p>
                <p className="text-xs text-indigo-300 mt-2">Yield Differential Forecast (12m)</p>
              </div>
              <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10 col-span-2">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-indigo-200 text-sm">Implied Volatility (VIX)</p>
                  <span className="text-rose-300 text-xs font-bold bg-rose-500/20 px-2 py-1 rounded">RISING</span>
                </div>
                <div className="w-full bg-indigo-900/50 rounded-full h-2 mb-2">
                  <div className="bg-rose-400 h-2 rounded-full w-[75%] shadow-[0_0_10px_rgba(251,113,133,0.5)]"></div>
                </div>
        ...
        <section className="max-w-5xl mx-auto px-6 pt-12 pb-8 -mt-20 relative z-20">
          <div className="rounded-2xl overflow-hidden shadow-2xl border border-slate-200">
            <img 
              src="https://i.imgur.com/oyqfzgL.jpeg" 
              alt="Grey Rhino Monetary Divergence Infographic" 
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 pb-20">
          ...
          {currentArticle?.googleDoc && (
            <div className="bg-gradient-to-r from-green-50 to-blue-50 p-8 rounded-xl my-12 text-center">
        ...
        <footer className="bg-white border-t border-slate-200 py-12">
          <div className="max-w-7xl mx-auto px-6 text-center text-slate-400 text-sm">
            ...
            <p>Data is simulated for illustrative purposes based on the &quot;Bessent Hypothesis&quot; research.</p>
          </div>
        </footer>
      </div>
    </>

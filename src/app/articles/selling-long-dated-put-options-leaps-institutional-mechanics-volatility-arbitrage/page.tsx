...
const Badge = ({ children, color = "blue" }: { children: React.ReactNode; color?: string }) => {
  const colorClasses: Record<string, string> = {
    ...
    slate: "bg-slate-100 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800",
  };
  return (
...
const SectionHeader = ({ icon: Icon, title, subtitle, colorClass }: { icon: React.ElementType; title: string; subtitle?: string; colorClass: string }) => (
  <div className="mb-8 border-b border-slate-200 dark:border-slate-800 pb-4">
    <div className={`flex items-center gap-3 mb-2 ${colorClass}`}>
      ...
      <h2 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-slate-200 font-serif">{title}</h2>
    </div>
...
const Card = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <div className={`bg-white dark:bg-[#0A0D14] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden ${className}`}>
    {children}
  </div>
);
...
const HighlightBox = ({ title, children, type = "info" }: { title?: string; children: React.ReactNode; type?: string }) => {
  const styles: Record<string, string> = {
    info: "bg-[#A8672E]/10 dark:bg-[#D08F52]/10 border-blue-200 text-blue-900",
    warning: "bg-amber-50 border-amber-200 text-amber-900",
    danger: "bg-[#BC4128]/10 dark:bg-[#E2694A]/10 border-rose-200 text-rose-900",
    success: "bg-[#1D8A70]/10 dark:bg-[#3CBF9C]/10 border-emerald-200 text-emerald-900",
    neutral: "bg-slate-50 dark:bg-[#14171B] border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100",
  };
  return (
...
export default function LeapStrategyDoc() {
  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto px-4 md:px-6 text-slate-600 dark:text-slate-400">
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          <span className="flex items-center gap-2 px-3 py-1 bg-[#1D8A70]/10 dark:bg-[#3CBF9C]/10 text-[#1D8A70] dark:text-[#3CBF9C] rounded-full text-sm font-medium border border-emerald-100">
          ...
          <span className="flex items-center gap-2 px-3 py-1 bg-[#BC4128]/10 dark:bg-[#E2694A]/10 text-[#BC4128] dark:text-[#E2694A] rounded-full text-sm font-medium border border-rose-100">
        ...
        <section className="mb-20 pt-4">
          ...
          <div className="grid gap-8">
            <Card className="p-6 md:p-10">
              <div className="flex flex-col md:flex-row gap-8 items-start">
                <div className="flex-1">
                  <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-200 mb-4 font-serif">The &ldquo;Why&rdquo;: Defining the Strategy</h3>
                ...
                <div className="w-full md:w-1/3 bg-slate-50 dark:bg-[#14171B] p-5 rounded-xl border border-slate-200 dark:border-slate-800">
                  <h4 className="font-bold text-slate-700 dark:text-slate-300 mb-3 border-b border-slate-200 dark:border-slate-800 pb-2">Primary Goals</h4>
                  <ul className="space-y-4">
                    <li className="flex gap-3">
                      <div className="mt-1">
                        <Target className="w-5 h-5 text-[#1D8A70] dark:text-[#3CBF9C]"/>
                      </div>
                      <div>
                        <strong className="block text-[#1D8A70] dark:text-[#3CBF9C]">Strategy A: Acquisition</strong>
                        <span className="text-sm text-slate-600 dark:text-slate-400">&ldquo;Buffett-Style&rdquo; Synthetic Limit Order. Goal is assignment at a discount.</span>
                      </div>
                    </li>
                    <li className="flex gap-3">
                      <div className="mt-1">
                      ...
                      <div>
                        ...
                        <span className="text-sm text-slate-600 dark:text-slate-400">&ldquo;Short Vega&rdquo; Play. Goal is mean reversion of high Implied Volatility (IV).</span>
                      </div>
                    </li>
                  </ul>
                </div>
            ...
            <Card className="overflow-hidden">
              <div className="bg-[#A8672E]/10 dark:bg-[#D08F52]/10 p-5 border-b border-indigo-100 flex justify-between items-center cursor-pointer" onClick={() => toggleSection('greek-profile')}>
                <h3 className="font-bold text-indigo-900 flex items-center gap-2 text-lg font-serif">
                  <Scale className="w-5 h-5" />
                  Comparative Greek Profile: 2-Year vs 30-Day
                </h3>
                {openSection === 'greek-profile' ? <ChevronUp className="text-[#A8672E] dark:text-[#D08F52]"/> : <ChevronDown className="text-[#A8672E] dark:text-[#D08F52]"/>}
              </div>
                  ...
                  <table className="w-full text-left text-sm md:text-base">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-[#14171B] border-b border-slate-200 dark:border-slate-800">
                        <th className="p-4 font-semibold text-slate-700 dark:text-slate-300">Metric</th>
                        <th className="p-4 font-semibold text-[#A8672E] dark:text-[#D08F52] bg-[#A8672E]/10 dark:bg-[#D08F52]/10/50 w-1/3">2-Year ATM Put (LEAP)</th>
                        <th className="p-4 font-semibold text-slate-500 w-1/3">30-Day ATM Put</th>
                        <th className="p-4 font-semibold text-slate-700 dark:text-slate-300">Strategic Implication</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      <tr>
                        <td className="p-4 font-medium text-slate-900 dark:text-slate-100">Vega (<span className="font-serif italic">ν</span>)</td>
                        <td className="p-4 bg-[#A8672E]/10 dark:bg-[#D08F52]/10/30 font-bold text-[#A8672E] dark:text-[#D08F52]">Extremely High (~$2.50)</td>
                        <td className="p-4 text-slate-500">Low (~$0.40)</td>
                        <td className="p-4 text-slate-600 dark:text-slate-400">LEAP P/L is driven by changes in Volatility. A 1% drop in IV creates massive profit.</td>
                      </tr>
                      <tr>
                        <td className="p-4 font-medium text-slate-900 dark:text-slate-100">Theta (<span className="font-serif italic">Θ</span>)</td>
                        <td className="p-4 bg-[#A8672E]/10 dark:bg-[#D08F52]/10/30 text-[#A8672E] dark:text-[#D08F52]">Very Low / Linear (~$0.40/day)</td>
                        <td className="p-4 font-bold text-slate-500">High / Exponential (~$9.00/day)</td>
                        <td className="p-4 text-slate-600 dark:text-slate-400">LEAPs are terrible for &ldquo;daily income.&rdquo; You are not paid to wait; you are paid for risk.</td>
                      </tr>
                      <tr>
                        <td className="p-4 font-medium text-slate-900 dark:text-slate-100">Gamma (<span className="font-serif italic">Γ</span>)</td>
                        <td className="p-4 bg-[#A8672E]/10 dark:bg-[#D08F52]/10/30 text-[#A8672E] dark:text-[#D08F52]">Stable / Low (0.001)</td>
                        <td className="p-4 text-slate-500">Explosive / High (0.008)</td>
                        <td className="p-4 text-slate-600 dark:text-slate-400">LEAPs are stable. Short-term puts have &ldquo;Gamma Risk&rdquo; (rapid losses during crash).</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
        ...
        <section className="mb-20">
          ...
          <div className="grid gap-6">
            <div className="grid md:grid-cols-2 gap-6">
              <HighlightBox type="success" title="Case Study 1: The Buffett Put (1993)">
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-[#1D8A70] dark:text-[#3CBF9C] font-bold mt-2">
            ...
            <Card className="p-6 md:p-8 bg-slate-50 dark:bg-[#14171B] border-none">
              ...
              <div className="grid md:grid-cols-3 gap-4 text-sm">
                <div className="bg-white dark:bg-[#0A0D14] p-4 rounded-lg shadow-sm">
                  <strong className="block text-slate-800 dark:text-slate-200 mb-1">Step 1: The Buy</strong>
                  Arb buys Stock + Deep ITM Put.
                </div>
                <div className="bg-white dark:bg-[#0A0D14] p-4 rounded-lg shadow-sm">
                  <strong className="block text-slate-800 dark:text-slate-200 mb-1">Step 2: The Dividend</strong>
                  Arb collects the dividend payment.
                </div>
                <div className="bg-white dark:bg-[#0A0D14] p-4 rounded-lg shadow-sm">
                  <strong className="block text-slate-800 dark:text-slate-200 mb-1">Step 3: The Exercise</strong>
                  Arb exercises Put to sell stock at fixed strike.
                </div>
              </div>
              <p className="mt-4 text-xs text-slate-500 bg-white dark:bg-[#0A0D14] p-2 rounded border border-slate-200 dark:border-slate-800 inline-block">
        ...
        <section className="mb-20">
          ...
          <div className="space-y-8">
            {/* Trap 1: Illiquidity */}
            <Card className="overflow-hidden">
              <div className="bg-[#BC4128]/10 dark:bg-[#E2694A]/10 p-5 border-b border-rose-100">
                <h3 className="text-xl font-bold text-rose-900 flex items-center gap-2 font-serif">
                  <ShieldAlert className="w-6 h-6"/> Trap 1: The Illiquidity &amp; Slippage Cost
                </h3>
                <p className="text-[#BC4128] dark:text-[#E2694A] mt-1">Unlike monthly options, LEAPs have massive bid-ask spreads.</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm md:text-base">
                  <thead className="bg-slate-50 dark:bg-[#14171B] text-slate-500 text-xs uppercase font-bold">
                    <tr>
                      <th className="p-4">Ticker / Type</th>
                      <th className="p-4">Expiration</th>
                      <th className="p-4">Bid / Ask</th>
                      <th className="p-4">Spread Cost ($)</th>
                      <th className="p-4 text-[#BC4128] dark:text-[#E2694A]">Slippage % (One-Way)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    <tr>
                      ...
                      <td className="p-4 text-slate-600 dark:text-slate-400">$78</td>
                      <td className="p-4 font-bold text-[#BC4128] dark:text-[#E2694A]">~1.99%</td>
                    </tr>
                    <tr>
                      ...
                      <td className="p-4 text-slate-600 dark:text-slate-400">$135</td>
                      <td className="p-4 font-bold text-[#BC4128] dark:text-[#E2694A]">~1.52%</td>
                    </tr>
                    <tr className="bg-[#BC4128]/10 dark:bg-[#E2694A]/10/30">
                      ...
                      <td className="p-4 text-slate-600 dark:text-slate-400">$240</td>
                      <td className="p-4 font-bold text-[#BC4128] dark:text-[#E2694A]">~13.08% !</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div className="p-4 bg-slate-50 dark:bg-[#14171B] text-sm text-slate-600 dark:text-slate-400 italic border-t border-slate-200 dark:border-slate-800">
            ...
            <Card className="p-6 md:p-8">
              ...
              <div className="grid gap-4">
                <div className="flex flex-col md:flex-row items-center justify-between p-4 bg-[#1D8A70]/10 dark:bg-[#3CBF9C]/10 rounded-xl border border-emerald-100 transition-transform hover:scale-[1.01]">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center font-bold text-[#1D8A70] dark:text-[#3CBF9C]">1</div>
                    <div>
                  ...
                  <div className="text-right mt-2 md:mt-0">
                    <span className="font-bold text-2xl text-[#1D8A70] dark:text-[#3CBF9C]">+$1,611</span>
                    <span className="block text-xs text-[#1D8A70] dark:text-[#3CBF9C]">Vega Profit: +$1,520</span>
                  </div>
                </div>

                <div className="flex flex-col md:flex-row items-center justify-between p-4 bg-[#BC4128]/10 dark:bg-[#E2694A]/10 rounded-xl border border-rose-100 transition-transform hover:scale-[1.01]">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center font-bold text-[#BC4128] dark:text-[#E2694A]">2</div>
                    <div>
                  ...
                  <div className="text-right mt-2 md:mt-0">
                    <span className="font-bold text-2xl text-[#BC4128] dark:text-[#E2694A]">-$1,189</span>
                    <span className="block text-xs text-[#BC4128] dark:text-[#E2694A]">Vega Loss: -$1,280</span>
                  </div>
                </div>
              </div>
            ...
            {/* Trap 3: Dead Money */}
            <Card className="p-6 bg-slate-50 dark:bg-[#14171B] border-dashed border-2 border-slate-300">
              <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300 mb-4 font-serif">Trap 3: Capital Inefficiency (&ldquo;Dead Money&rdquo;)</h3>
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  <h5 className="font-semibold text-slate-600 dark:text-slate-400 mb-2">Strategy: LEAP Put</h5>
                  <div className="h-2 w-full bg-slate-200 rounded-full mb-2 overflow-hidden">
                    <div className="h-full bg-[#A8672E] dark:bg-[#D08F52] w-[18%]"></div>
                  </div>
                  <p className="text-2xl font-bold text-[#A8672E] dark:text-[#D08F52]">18.1% ROC</p>
                  <p className="text-xs text-slate-500">Max Return on Capital (2 Years)</p>
                </div>
                <div>
                  <h5 className="font-semibold text-slate-600 dark:text-slate-400 mb-2">Strategy: Monthly Puts</h5>
                  <div className="h-2 w-full bg-slate-200 rounded-full mb-2 overflow-hidden">
                    <div className="h-full bg-[#1D8A70] dark:bg-[#3CBF9C] w-[90%]"></div>
                  </div>
                  <p className="text-2xl font-bold text-[#1D8A70] dark:text-[#3CBF9C]">130.9% ROC</p>
        ...
        <section className="mb-4">
          ...
          <div className="grid lg:grid-cols-3 gap-6 mb-8">
            <Card className="p-6 border-t-4 border-t-emerald-500 hover:-translate-y-1 transition-transform duration-300">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-2 font-serif">OTM Strike</h3>
            ...
            <Card className="p-6 border-t-4 border-t-amber-500 hover:-translate-y-1 transition-transform duration-300">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-2 font-serif">ATM Strike</h3>
            ...
            <Card className="p-6 border-t-4 border-t-blue-500 hover:-translate-y-1 transition-transform duration-300">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-2 font-serif">Deep ITM</h3>
          ...
          <Card className="p-8 bg-slate-900 text-slate-200">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-white mb-2 font-serif">The &ldquo;No-Exit&rdquo; Rule</h3>
                <p className="mb-6 text-slate-400">
                  Due to the liquidity trap, you must assume you are married to this position for 2 years.
                </p>
                <div className="grid md:grid-cols-3 gap-6">
                  <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
                    <div className="w-8 h-8 rounded-full bg-[#1D8A70] dark:bg-[#3CBF9C]/20 text-[#1D8A70] dark:text-[#3CBF9C] flex items-center justify-center font-bold text-sm mb-2">A</div>
                  ...
                  <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
                    <div className="w-8 h-8 rounded-full bg-[#A8672E] dark:bg-[#D08F52]/20 text-[#A8672E] dark:text-[#D08F52] flex items-center justify-center font-bold text-sm mb-2">B</div>
                  ...
                  <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
                    <div className="w-8 h-8 rounded-full bg-[#BC4128] dark:bg-[#E2694A]/20 text-[#BC4128] dark:text-[#E2694A] flex items-center justify-center font-bold text-sm mb-2">C</div>

    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
                <div className="inline-flex rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-100/80 dark:bg-gray-800/80 p-0.5 text-xs">
                ...
                {loading && <span className="text-xs font-mono text-slate-500 dark:text-slate-400">Loading…</span>}
                {error && <span className="text-xs font-mono text-rose-600 dark:text-rose-400">{error} — showing last loaded data.</span>}
                <div className="flex items-center gap-2.5 ml-auto">
                    <span className="text-xs font-mono font-medium text-slate-500 dark:text-slate-400">Spot: <strong className="text-slate-900 dark:text-slate-100">${snapshot.underlyingPrice.toLocaleString()}</strong></span>
                    {source === 'live' && (
                        <button
                            onClick={() => loadSource('live', true)}
                            disabled={loading}
                            title="Refresh live SPX quotes"
                            className="p-1.5 rounded-lg border border-gray-200 dark:border-gray-800 text-slate-500 dark:text-slate-400 hover:text-[#A8672E] dark:hover:text-[#D08F52] hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors disabled:opacity-50"
                        >
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`p-1.5 rounded-lg border transition-colors ${assumptionsOpen ? 'border-[#A8672E] text-[#A8672E] bg-[#A8672E]/10' : 'border-gray-200 dark:border-gray-800 text-slate-500 dark:text-slate-400 hover:text-[#A8672E] dark:hover:text-[#D08F52]'}`}
                        >
                            <Settings2 className="h-3.5 w-3.5" />
                        </button>
                        {assumptionsOpen && (
                            <div className="absolute right-0 top-full mt-2 z-20 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-lg p-4 w-60 text-xs space-y-3">
                                <p className="text-[11px] font-serif font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wide">Pricing Assumptions</p>
                                <label className="flex items-center justify-between gap-2 font-mono">
                                    Rate
                                    <span className="flex items-center gap-1">
                                        <input type="number" step={0.1} className="w-16 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200" value={ratePct} onChange={e => setRatePct(Number(e.target.value) || 0)} />%
                                    </span>
                                </label>
                                <label className="flex items-center justify-between gap-2 font-mono">
                                    Div yield
                                    <span className="flex items-center gap-1">
                                        <input type="number" step={0.1} className="w-16 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200" value={divYieldPct} onChange={e => setDivYieldPct(Number(e.target.value) || 0)} />%
                                    </span>
                                </label>
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 pt-2 border-t border-gray-100 dark:border-gray-800">Position size in $ = per-contract × 100 (SPX multiplier).</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label className="block text-xs font-serif font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Preset</label>
                    {lockPreset ? (
                        <div className="w-full border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800/50 text-slate-800 dark:text-slate-200 font-medium">
                            {preset.label}
                        </div>
                    ) : (
                        <select
                            className="w-full border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2 text-sm bg-white dark:bg-gray-900 text-slate-800 dark:text-slate-200"
                            value={presetId}
                ...
                <div>
                    <label className="block text-xs font-serif font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Expiration</label>
                    <select
                        className="w-full border border-gray-200 dark:border-gray-800 rounded-xl px-3 py-2 text-sm bg-white dark:bg-gray-900 text-slate-800 dark:text-slate-200 font-mono"
                        value={expiration}
            ...
            {dirty && (
                <div className="flex items-center justify-between text-xs bg-[#A8672E]/10 border border-[#A8672E]/30 rounded-xl px-3.5 py-2.5">
                    <span className="text-[#A8672E] dark:text-[#D08F52] font-medium">Legs have been hand-edited — they'll carry over if you change the expiration or source.</span>
                    <button onClick={resetToPreset} className="font-semibold text-[#A8672E] dark:text-[#D08F52] underline hover:no-underline flex-shrink-0 ml-3">
                        Reset to preset
                    </button>
                </div>
            )}
            ...
            <div className="overflow-x-auto border border-gray-200 dark:border-gray-800 rounded-2xl bg-white dark:bg-gray-900 shadow-xs">
                <table className="w-full text-sm font-mono">
                    <thead className="bg-gray-50/70 dark:bg-gray-800/40 text-xs font-serif font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide border-b border-gray-100 dark:border-gray-800">
                        <tr>
                            <th className="text-left px-3 py-2.5">Side</th>
                            <th className="text-left px-3 py-2.5">Qty</th>
                            <th className="text-left px-3 py-2.5">Type</th>
                            <th className="text-left px-3 py-2.5">Strike</th>
                            <th className="text-left px-3 py-2.5">Price</th>
                            <th className="text-left px-3 py-2.5">IV</th>
                            <th className="text-left px-3 py-2.5">Delta</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {legs.map((leg, i) => {
                            ...
                            return (
                                <tr key={leg._id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                                    <td className="px-3 py-2">
                                        <select
                                            className="border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-xs bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200 font-semibold"
                                            value={leg.side}
                                    ...
                                    <td className="px-3 py-2">
                                        <input
                                            type="number" min={1}
                                            className="w-14 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-xs bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200 font-mono"
                                        ...
                                        {isStock ? (
                                            <div className="flex items-center gap-1">
                                                ...
                                                <input
                                                    ...
                                                    className="w-20 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-xs bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200 font-mono"
                                                    value={leg.strike}
                                            ...
                                            <select
                                                className="border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-xs bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200 font-mono"
                                                value={leg.strike}
                                                onChange={e => {
                                    ...
                                    <td className="px-3 py-2">
                                        {isStock ? (
                                            <span className="text-xs text-slate-400 font-mono">— (no premium)</span>
                                        ) : (
                                            <input
                                                type="number" step={0.01}
                                                className="w-20 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-xs bg-white dark:bg-gray-800 text-slate-800 dark:text-slate-200 font-mono"
                                    ...
                                    <td className="px-3 py-2 text-xs font-mono text-slate-600 dark:text-slate-400">{isStock ? '—' : priced ? fmtPct(priced.iv) : '—'}</td>
                                    <td className="px-3 py-2 text-xs font-mono text-slate-600 dark:text-slate-400">{isStock ? '1.00' : selected ? Math.abs(selected.delta).toFixed(2) : '—'}</td>
                                    <td className="px-3 py-2">
                                        <button onClick={() => removeLeg(leg._id)} className="text-xs text-rose-600 hover:text-rose-800 p-1" aria-label="Remove leg">✕</button>
                                    </td>
                                </tr>
                            );
                        })}
                        {legs.length === 0 && (
                            <tr><td colSpan={8} className="px-3 py-4 text-center text-xs font-mono text-slate-400">No legs — add one below.</td></tr>
                        )}
                    </tbody>
                </table>
                <div className="relative border-t border-gray-100 dark:border-gray-800">
                    <button
                        ...
                        className="w-full text-xs font-semibold text-[#A8672E] dark:text-[#D08F52] hover:bg-[#A8672E]/10 py-2.5 flex items-center justify-center gap-1 transition-colors"
                    >
                        + Add leg <ChevronDown className="h-3.5 w-3.5" />
                    </button>
                    {addMenuOpen && (
                        <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 z-10 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-lg overflow-hidden text-xs w-36">
                            <button onClick={() => addLeg('call')} className="block w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-slate-700 dark:text-slate-300 font-semibold">+ Call</button>
                            <button onClick={() => addLeg('put')} className="block w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-slate-700 dark:text-slate-300 font-semibold border-t border-gray-100 dark:border-gray-800">+ Put</button>
                            <button onClick={() => addLeg('stock')} className="block w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-slate-700 dark:text-slate-300 font-semibold border-t border-gray-100 dark:border-gray-800">+ Stock</button>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden bg-white dark:bg-gray-900 shadow-xs">
                    ...
                    <div className="p-4">
                        ...
                        {analysisTab === 'probability' && atmIV !== null && (
                            <div>
                                {canSnapToBoundary && (
                                    <div className="flex flex-wrap items-center justify-end gap-1.5 text-xs mb-3 font-mono">
                                        <span className="text-slate-400">Set short strikes to:</span>
                                        <button onClick={() => snapToBoundary(0.68)} className="px-2.5 py-1 rounded-lg border border-[#A8672E]/30 text-[#A8672E] dark:text-[#D08F52] bg-[#A8672E]/10 hover:bg-[#A8672E]/20 font-semibold">68% boundary</button>
                                        <button onClick={() => snapToBoundary(0.80)} className="px-2.5 py-1 rounded-lg border border-[#A8672E]/30 text-[#A8672E] dark:text-[#D08F52] bg-[#A8672E]/10 hover:bg-[#A8672E]/20 font-semibold">80% boundary</button>
                                    </div>
                                )}

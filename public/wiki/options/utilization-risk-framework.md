...
- **Contract Anatomy** — A ticker like `SPY 240119 C 480.00` encodes the underlying, expiration date, type (Call/Put), and strike price; one contract controls 100 shares.
- **Opening vs. Closing** — Buy To Open (BTO) / Sell To Open (STO) establish a position; Sell To Close (STC) / Buy To Close (BTC) exit one. STO carries undefined or high max risk since it's an obligation, not a right.
